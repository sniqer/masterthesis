...
//		basicCalc.removePotentialErrorsInSINR(SINR);
	...
	double[] realThroughputPerMs = basicCalc.tbs2throughput(ulTbsSum,ndf,timeStampInDigits,1000);
	double[][] test2 = getValuesOverTime(realThroughputPerMs,timeStampInDigits,1000);
...
//		Print.array(realThroughputPerMs);
	...
	CorruptedParameter.SINR(SINR);
	CorruptedParameter.timeStamp(timeStampInDigits);
...
public static int[] interpretLTEdata(String[] lteData){
	...
	public static double[][] getValuesOverTime(double[] values, double[] timeStampInDigits,int timeConstant){
